...
#IN THIS DO FILE
  ...
  #2. ESTIMATE SPEAKER NUMBERS IN THE YEAR 2001
  #3. TEST WHETHER ALL COUNTS ARE LIKELY TO COME FROM THE SAME POPULATION
  #4. TEST FOR THE CONSISTENCY OF THE AGE PROFILES
...
#mortality probabilities (concentrating on the registered Indian population)
m <- lt %>% 
  filter(Location=="Upper-middle-income countries", Time %in% seq(2001, 2096, 5), AgeGrpStart!=0) %>%
...
m <- (m[-21, -20]+m[-1, -1])/2
m <- rbind(m, matrix(rep(1, 19), ncol=19))
...
m0 <- c(
  lt %>% 
    ...
    pull(qx)+ 
    lt %>% 
    filter(Location=="Upper-middle-income countries", Time %in% seq(2001, 2096, 5), AgeGrpStart==1) %>%
    pull(qx)*(2/3))[-20]

#fertility rates
f <- ft %>% 
  filter(Location=="Lower-middle-income countries", Time %in% seq(2001, 2096, 5), AgeGrpStart<=45) %>%
  pull(ASFR) %>%
  matrix(nrow=8)

#fill the rest of the fertility matrix with zeros
f <- rbind(matrix(nrow=2, ncol=20, 0), f, matrix(nrow=11, ncol=20, 0))/400
...
names_years <- sc %>% filter(year!=2001) %>% select(name, year) %>% unique()
...
backcast_fct <- function(l){
  ...
  lg <- length(seq(2006, yr, 5))
  
  p <- sc %>% filter(name == na, year == yr) %>% pull(pop) %>% list()
  
  for (i in 1:lg){
    
    p[[i+1]] <- c(p[[i]] + p[[i]]*m[ , lg-i+1], 0)[-1]
    
  }
  
  data.frame(name=na, age=seq(0, 80, 5), census=yr, n=p[[i+1]][1:17])
  
}

#run function
n01 <- bind_rows(lapply(1:length(names_years$name), function(x) backcast_fct(x)))
