################################################################################
#In this do file
#Comparison, Canadian census data, Ethnologue data, & ELP data
################################################################################
################################################################################
...
setwd("C:/Users/micbo262/Documents/DataSpeakerNumbers/")

#Year 2001######################################################################
#load data, select appropriate information, add age vector
sc01 <- read.csv("Canada/Census/indigenousmothertongue2001.csv")[5:39, -2] %>% 
...
#Year 2006######################################################################
#load data, select appropriate information, add age vector
sc06 <- read.csv("Canada/Census/indigenousmothertongue2006.csv")[5:40, -2] %>% 
...
#Year 2011######################################################################
sc11 <- read.csv("Canada/Census/indigenousmothertongue2011.csv")[7:80, -2] %>% 
  pivot_longer(X0.to.4.years:X100.years.and.over, names_to="age", values_to="pop") %>%
  mutate(age = str_extract(age, "[0-9]+")) %>%
  rename(name = 1) %>%
  filter(!str_detect(name, "languages") | (str_detect(name, "n.i.e.") | str_detect(name, "n.o.s."))) %>%
...
#Year 2016######################################################################
sc16 <- read.csv("Canada/Census/indigenousmothertongue2016.csv")[8:89, -2] %>% 
  pivot_longer(X0.to.4.years:X100.years.and.over, names_to="age", values_to="pop") %>%
  mutate(age = str_extract(age, "[0-9]+")) %>%
  rename(name = 1) %>%
  filter(!str_detect(name, "languages") | (str_detect(name, "n.i.e.") | str_detect(name, "n.o.s."))) %>%
...
#Year 2021######################################################################
#load data
sc21 <- read.csv("Canada/Census/indigenousmothertongue2021.csv")[5:76, -2] %>% 
  pivot_longer(X0.to.4.years:X100.years.and.over, names_to="age", values_to="pop") %>%
  mutate(age = str_extract(age, "[0-9]+")) %>%
  rename(name = 1) %>%
  filter(!str_detect(name, "languages") | (str_detect(name, "n.i.e.") | str_detect(name, "n.o.s."))) %>%
...
sc <- sc %>% bind_rows(sc %>% 
                         ...
                         mutate(name = paste(continuum, "(continuum)")))

#identify languages or continua that appear 4 times or more
fourtimes <- sc %>% 
  filter(!is.na(pop), !is.na(year), !is.na(age), !grepl('languages', name)) %>% 
  distinct(name, year) %>% 
  group_by(name) %>%
  summarise(n = n()) %>%
  filter(n >=4) %>% 
...
#keep observations that appear 4 times, save
sc %>% 
  filter(name %in% fourtimes, !is.na(pop), !is.na(age)) %>%
  arrange(year, name, age) %>%
  saveRDS("C:/Users/micbo262/Documents/DataSpeakerNumbers/Canada/Analyses/scdata")
