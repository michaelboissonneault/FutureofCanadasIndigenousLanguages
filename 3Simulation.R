...
################################################################################
#counts in 1951
N <- list(rpois(1:21, (21:1)^2/30))
          
#intergenerational transmission ratio, whole period
itr <- 1 / (1+exp(1)^(.3*(1:30-10)))
...
ggplot(data.frame(it_predictions) %>% 
  ...
  geom_smooth(aes(year, prediction))
...
#pivot longer
ff_sum_long <- ff_sum %>% pivot_longer(c(q2, real), names_to = "pred_vs_real", values_to = 'value')

#show result
ggplot(ff_sum_long)+
  geom_line(aes(year, value, linetype = pred_vs_real, group = pred_vs_real), linewidth = 1.5)+
